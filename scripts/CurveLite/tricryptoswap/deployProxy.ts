...
export async function deployCurveLiteTricryptoswapProxy(deployer: Signer, crossChainLayerAddress: string, localTest: boolean = false): Promise<CurveLiteTricryptoswapProxy> {
    const CurveLiteTricryptoswapProxy = await deployUpgradable<CurveLiteTricryptoswapProxy>(deployer, hre.artifacts.readArtifactSync('CurveLiteTricryptoswapProxy'), [await deployer.getAddress(), crossChainLayerAddress], undefined, true);
