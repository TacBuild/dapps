...
async function deployProxy(treasuryAddress: string) {
    ...
    const tacContractsCrossChainLayer = "0x0928d67A277891832c743F8179bf2035D0025392";

    const proxy = await TreasurySwapProxy.deploy();
    
    await proxy.waitForDeployment();
    // init contract
    const tx = await proxy.initialize(deployer.address, treasuryAddress, wTON, tacContractsCrossChainLayer);
    await tx.wait();
...
async function main() {
    ...
    for (const token of tokens) {
        ...
        const tokenContract = (await ethers.getContractAt("TestnetERC20", tokenAddress)) as unknown as TestnetERC20;
