...
interface LiquidStakingI is authorization.AuthorizationI {
    ...
    // admin transactions
    function updateParams(LiquidStakeUpdatableParams calldata params) external returns (bool success);

    function updateWhitelistedValidators(
        WhitelistedValidator[] calldata whitelistedValidators
    ) external returns (bool success);

    function setModulePaused(bool isPaused) external returns (bool success);
    ...
    event LiquidStake(address indexed delegatorAddress, uint256 amount);
    ...
    event LiquidUnstake(address indexed delegatorAddress, uint256 amount);

    event UpdateParams(LiquidStakeUpdatableParams params);

    event UpdateWhitelistedValidator(WhitelistedValidator[] whitelistedValidators);

    event SetModulePaused(bool isPaused);
