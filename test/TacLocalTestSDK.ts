...
describe("TacLocalTestSDK", () => {
    ...
    it('Test send message', async () => {
        ...
        expect(outMessage.shardedId).to.be.eq(shardedId);
        ...
        receipt.logs.forEach((log) => {
            ...
            if (event && event.name === "InvokeWithCallback") {
                found = true;
                // event InvokeWithCallback(uint64 shardedId, uint256 timestamp, string operationId, string tvmCaller, bytes extraData, TokenAmount[] receivedTokens);
                const typedEvent = event as unknown as InvokeWithCallbackEvent.LogDescription;
                expect(typedEvent.args.shardedId).to.be.eq(shardedId);
